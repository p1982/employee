//Імпорт bcrypt для хешування паролів, що дозволяє безпечно зберігати та порівнювати паролі клієнтів
const bcrypt = require("bcrypt")
// Імпортуйте моделі баз даних для взаємодії з базою даних MongoDB, зокрема для операцій, пов'язаних з клієнтами
...
router.post("/signin", async (req, res) => {
    try {
        // Спроба знайти існуючого клієнта в базі даних за його email
        const foundClient = await db.Clients.findOne({ email: req.body.email })
        // Якщо клієнта не знайдено, повідомити клієнту, що існуючого користувача не знайдено
        if (!foundClient) {
            ...
            // Якщо клієнт знайдений і надісланий пароль співпадає з хешованим паролем в базі даних
            //Зберегти інформацію про клієнта в сесії, щоб підтвердити, що він увійшов в систему 
            req.session.currentUser = foundClient
            // Після успішного входу перенаправляємо клієнта на сторінку його профілю
...
router.post('/signup', async (req, res) => {
    // Знайти за email існуючого клієнта, який намагається зареєструватися, щоб порівняти паролі
    try {
        // Спроба знайти існуючого клієнта в базі даних за його email
        const foundClient = await db.Clients.findOne({ email: req.body.email })
        // Якщо клієнт вже зареєстрований з даним email, запропонувати йому зареєструватися замість нього
        if (foundClient) {
            ...
            // Виключити поле confirmPassword з тіла запиту перед створенням нового запису клієнта
            const { confirmPassword, ...rest } = req.body
            // Створити новий запис про клієнта в базі даних з хешованим паролем.
            const newClient = await db.Clients.create({
                ...rest, password: hashedString
            })
            // Зареєструвати нового клієнта, зберігши його дані в сесії
            req.session.currentUser = newClient
            // Перенаправлення клієнта на сторінку його профілю після успішної реєстрації
            res.redirect('/profile')
        }
    }
    catch (err) {
